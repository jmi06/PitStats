...
def gatherData():
    ...
    for series in series_num:
        for year in years:
            ...
            print(f'https://cf.nascar.com/cacher/{year}/{series}/final/{series}-drivers-points.json')
            ...
            for i in driver_standings_json:
                ...
                if drivers_name in drivers:
                    print('sum', sum(1 for race in races.values() if race))
                    print('race num', drivers[drivers_name]['race_num'])
                    if drivers[drivers_name]['race_num'] >= sum(1 for race in races.values() if race) -1:
                ...
                os.makedirs(f'data/{name_series}/{year}', exist_ok=True)
                with open(f'data/{name_series}/{year}/drivers.json', 'w') as driverFile:
                    json.dump(drivers, driverFile)

                with open(f'data/{name_series}/{year}/races.json', 'w') as raceFile:
